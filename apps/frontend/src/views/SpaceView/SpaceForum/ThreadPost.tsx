...
export const ThreadPost = ({ posts, space, profiles, myAddress }) => {
  const { postKey, threadKey, spaceKey } = useParams();
  ...
  const getReplies = useCallback((thread: any): Reply[] => {
    if (typeof thread === 'object') {
      return Object.values(thread).filter(
        (entry: any) => entry?.data?.type === 'comment',
      ) as Reply[];
    }
  ...
  useEffect(() => {
    if (typeof posts === 'object') {
      ...
      if (selectedPost) {
        ...
        const selectedComment = selectedPost.thread?.[threadKey];
        if (selectedComment) {
    ...
    <>
      <div id="content-right" className="relative float-right w-full pl-0 lg:w-3/4 lg:pl-5">
        ...
        <div className="px-4 md:px-0">
          <h1 className="mb-4">Post comment</h1>
        </div>
        {!comment || loading ? (
          <PageLoader />
        ) : (
          <>
            ...
            {comment.subThread && (
              <div className="my-4 space-y-4">
                {memoizedReplies.map((reply: Reply, i) => (
                  <div key={`ReplyItem_${i}`}>
                    <CommentThreadItem
                      ...
                      commentId={threadKey ? threadKey : undefined}
                    />
                  </div>
                ))}
              </div>
